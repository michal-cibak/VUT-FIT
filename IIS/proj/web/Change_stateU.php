<?php
    ...
    if (array_key_exists('id', $_GET)) {
        try {
            $stmt = $pdo->prepare("SELECT * FROM Rezervace_uzivatele WHERE id = ?");
        ...
        echo "<table>
            <tr>
            <th>ID uživatele</th><td>${uzivatel['uzivatel_id']}</td>
            </tr>
            <tr>
        ...
        if($_POST)
        {
            ...
            try {
                $stmt = $pdo->prepare("UPDATE Rezervace_uzivatele SET stav = ? WHERE id = ?");
                $stmt->execute(array($_POST['stav'], $uzivatel['id']));
            }
            catch (PDOException $e)
            {
                echo '<h2>Spojení s databází selhalo: ' .$e->getMessage().'</h2>';
            ...
            {
                echo "${_POST['stav']}</td>";
                //uzivatel nezaplatil jeho zadanka je smazana a vstupenky jsou znovu k dispozici
                if($_POST['stav']=='zamitnuto')
                {
                    try {
                        $stmt = $pdo->prepare("DELETE FROM Vstupenka Where rezervace_uzivatele_id = ?");
                        $stmt->execute(array($uzivatel['uzivatel_id']));
                        $pocet = $stmt->rowCount();
        ...
        echo "<form method='POST' action='Change_stateU.php?id=${uzivatel['id']}'>
